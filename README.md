...
cargo run --release -- encode input.png -o output.pixlzr
cargo run --release -- decode output.pixlzr -o restored.png
cargo run --release -- video-filter --input clip.mp4 --fps 60
```
...
```
Input image → Density map → Quantize → Entropy encode → .pixlzr
                                                        ↓
                                              Decode ← Entropy decode
                                                        ↓
                                              Output ← Dequantize + reconstruct
```
...
- Magic: `PIXLZR` + version byte
- Header: width, height, density-map params, quant tables
- Payload: density-weighted blocks, entropy-coded
...
Guilherme Zaluchi — [@guiga-zalu](https://github.com/guiga-zalu) · [LinkedIn](https://linkedin.com/in/guilherme-alves-c-zaluchi) · Ribeirão Pires, SP (Remote)

> **Copy instructions:** This `README.md` is a drop-in EN default. Preserve the current PT-BR README as `README.pt-BR.md` (add `> **Português (BR):** ...` header). See `docs/pixlzr-readme-patch.md §0` in the source pipeline repo for moves.
